...
//警告：	在预览请求回调函数，仅允许调用调用NET_ESTREAM_SetPreviewDataCB或NET_ESTREAM_SetStandardPreviewDataCB接口，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
//警告：	严禁在回放数据回调函数中调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
//警告：	在回放请求回调函数中调用，仅允许调用NET_ESTREAM_SetPlayBackDataCB接口，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
//警告：	在语音对讲数据回调函数中严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
//警告：	在语音对讲请求回调函数中，仅允许调用NET_ESTREAM_SetVoiceTalkDataCB接口，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
