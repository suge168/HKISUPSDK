...
//初始化，反初始化
NET_DVR_API BOOL  CALLBACK NET_ECMS_Init();//警告：该接口调用之前可以调用NET_ECMS_SetSDKInitCfg接口，其他接口均需要在该接口之后调用。
...
//警告：	在注册回调函数中，仅允许调用NET_ECMS_SetDeviceSessionKey、NET_ECMS_GetDeviceSessionKey接口，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
//警告：	在注册回调函数中，应避免执行耗时操作，否则将影响HCISUPSDK内部的任务处理效率，进而导致业务异常。 建议平台应用在注册回调中的耗时时间不宜超过10ms。
...
//开启关闭监听
//警告：在注册回调函数中，仅允许调用NET_ECMS_SetDeviceSessionKey、NET_ECMS_GetDeviceSessionKey接口，其他HCISUPSDK外部接口严禁调用，否则可导致程序崩溃或堵死。
...
//注销设备
//当平台端需要删除设备时，可以调用此接口。其他情形不建议调用此接口。
//当CMS注册回调中产生回调ENUM_DEV_OFF时，严禁调用本接口，否则会导致程序堵死。
...
//警告：	在中心管理服务器（CMS）的异步回调函数中，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
//警告：	在中心管理服务器（CMS）的异步回调函数中，应避免执行耗时操作，否则将影响HCISUPSDK内部的任务处理效率，进而导致业务异常。平台应用在回调中的耗时时间不宜超过10ms。
...
typedef struct tagNET_EHOME_DEV_REG_INFO
{
    ...
    //第六位为1表示支持延时摄影回放(0b00100000)
    //第七位为1表示支持语音对讲链路纯监听传输—即只有上行数据无下行数据(0b01000000)
    BYTE   byWakeupMode;                  //设备休眠模式 0-非携带式唤醒,默认值 1-携带式唤醒,即快速唤醒
    BYTE   byRes[23];
...
typedef struct tagNET_EHOME_DEV_REG_INFO_V12
{
    ...
    BYTE   byFirmwareIdenCode[MAX_FIRMWARE_IDENT_CODE_LEN]; //设备固件识别码
...
typedef struct tagNET_EHOME_SERVER_INFO
{
    ...
    NET_EHOME_BLACKLIST_SEVER   struBlackListServer;    //非授权名单服务器
    BYTE                        byRes[128];
...
typedef struct tagNET_EHOME_SERVER_INFO_V50
{
    ...
    BYTE                    byChildDeviceFlag;      //是否支持子设备透传报警：0-否，1-是
    BYTE                    byRes[367];
...
//警告：	严禁在音频数据回调函数中调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
typedef struct tagNET_EHOME_VOICE_TALK_IN
{
    ...
    BYTE  byListen; //语音监听标识, 设备接收到本标识为1后只进行音频采集发送给对端
    BYTE  byRes1[2];
...
static const NET_DVR_TYPE_NAME DVRTypeName[] = 
{
    ...
    {62,"IDS6002_HF_B"},          //DS6002-HF/B
    {63,"IDS6101_HF_B"},          //异常行为检测：DS6101-HF/B
...
//警告：	严禁在透传数据异步回调函数中调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
//警告：	在透传数据异步回调函数中，应避免执行耗时操作，否则将影响HCISUPSDK内部的任务处理效率，进而导致业务异常。平台应用在回调中的耗时时间不宜超过10ms。
...
//警告：	严禁在异常回调函数中调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
//警告：	严禁在长连接回调函数中调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
...
typedef struct tagNET_EHOME_SYSCOMPONENT_REDUCED_ADDR
{
    ...
    BYTE    szComponentPkgName[32]; //组件包名
...
//警告：	在NET_ECMS_XMLConfig的异步回调函数中(ASYNC_RESPONSE_CB)，严禁调用其他HCISUPSDK外部接口，否则可导致程序崩溃或堵死。
//警告：	在NET_ECMS_XMLConfig的异步回调函数中(ASYNC_RESPONSE_CB)，应避免执行耗时操作，否则将影响HCISUPSDK内部的任务处理效率，进而导致业务异常。平台应用在回调中的耗时时间不宜超过10ms。
